 ...
 */
/** ***************************************************************************
 * LeftMaskedFeature Model (implementation)
 ******************************************************************************
 * $Author: dubik $
...
LeftMaskedFeature::LeftMaskedFeature(float x, float y, bool lost,
...
LeftMaskedFeature::~LeftMaskedFeature()
{
}
// ============================================================================

FeaturePtr LeftMaskedFeature::clone() const
{
    FeaturePtr copy(new LeftMaskedFeature(Feature::_x, Feature::_y, Feature::_lost,
                            Feature::_error));
...
float LeftMaskedFeature::compareTo(std::pair<float, float> pt,
                 const Image* img, const Image* mask)
{
    ...
    return (float)(sqrt(sumSqDiff));
}
// ============================================================================

bool LeftMaskedFeature::isFeature(std::pair<float, float> pt,
               const Image* img, const Image* mask)
{
    int height = mask->getHeight();
    int width = mask->getWidth();
...
void LeftMaskedFeature::generateDescriptor(std::pair<float, float> pt,
                             const Image *img, const Image *mask)
{
    int height = mask->getHeight();
    int width = mask->getWidth();
