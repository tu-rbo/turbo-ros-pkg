 ...
 */
#ifndef CUBE_CORNER_FEATURE_H
#define CUBE_CORNER_FEATURE_H

#include "feature.h"

namespace vision {
/**
 *\class    CubeCornerFeature
 *\brief
 *$Author: dubik $
 *$Date: 2009/03/10 15:00:46 $
 *$Revision: 1.1 $
 */
class CubeCornerFeature: public Feature {
public:
    /**
     * CubeCornerFeature constructor
     *
     ...
     */
    CubeCornerFeature(float x = 0, float y = 0, bool isLost = false,
            float error = 0.0, float quality = -1.0);

    /**
     * Creates a new CubeCornerFeature object with the same values as this one and
    ...
    virtual float compareTo(std::pair<float, float> pt, const Image* img,
            const Image* mask);

    virtual bool isFeature(std::pair<float, float> pt, const Image* img,
            const Image* mask);
