...
{
  void
  (*func)(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs,
      double *hx, void *adata); /* function to differentiate */
...
inline static void *
emalloc_(char *file, int line, size_t sz)
{
  void *ptr;
  ...
  if (ptr == NULL)
    {
      fprintf(
          stderr,
          "SBA: memory allocation request for %u bytes failed in file %s, line %d, exiting",
          sz, file, line);
      exit(1);
    }
...
inline static void
_dblzero(register double *arr, register int count)
{
  while (--count >= 0)
    *arr++ = 0.0;
}
...
static double
sba_mean_repr_error(int n, int mnp, double *x, double *hx,
  ...
  for (i = nprojs = 0, err = 0.0; i < n; ++i)
    {
      ...
      nprojs += nnz;
      for (j = 0; j < nnz; ++j)
        { /* point i projecting on camera rcsubs[j] */
          ptr1 = x + idxij->val[rcidxs[j]] * mnp;
          ptr2 = hx + idxij->val[rcidxs[j]] * mnp;

          err += sqrt(
              (ptr1[0] - ptr2[0]) * (ptr1[0] - ptr2[0]) + (ptr1[1] - ptr2[1])
                  * (ptr1[1] - ptr2[1]));
        }
    }
...
static void
sba_print_sol(int n, int m, double *p, int cnp, int pnp, double *x, int mnp,
    struct sba_crsm *idxij, int *rcidxs, int *rcsubs)
{
  register int i, j, ii;
  int nnz;
 ...
 */
static double
nrmL2xmy(double * const e, const double * const x, const double * const y,
    const int n)
{
 ...
 */
static double
nrmCxmy(double * const e, const double * const x, const double * const y,
    const double * const W, const int mnp, const int nvis)
{
  ...
  for (i = 0, Wptr = W, eptr = e, norm = 0.0; i < nvis; ++i, Wptr += mnpsq, eptr
      += mnp)
    {
      for (ii = 0, auxptr = Wptr; ii < mnp; ++ii, auxptr += mnp)
        { /* auxptr=Wptr+ii*mnp */
...
static void
sba_print_inf(double *hx, int nimgs, int mnp, struct sba_crsm *idxij,
      ...
      for (i = 0; i < nnz; ++i)
        {
          ...
          for (k = 0; k < mnp; ++k)
            if (!SBA_FINITE(phxij[k]))
              printf(
                  "SBA: component %d of the estimated projection of point %d on camera %d is invalid!\n",
                  k, rcsubs[i], j);
        }
    }
}
...
typedef int
(*PLS)(double *A, double *B, double *x, int m, int iscolmaj);
...
sba_motstr_levmar_x(
     ...
     */
    void
    (*fjac)(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs,
    ...
    double *Vout, double *Sout, double *Uout, double *Wout)
{
  ...
  /* submatrices sizes */
  int Asz, Bsz, ABsz, Usz, Vsz, Wsz, Ysz, esz, easz, ebsz, YWtsz, Wtdasz,
      Sblsz, covsz;
  ...
  *pdp; /* p + dp, size m*cnp + n*pnp */
  ...
  double tau = FABS(opts[0]), eps1 = FABS(opts[1]), eps2 = FABS(opts[2]),
      eps2_sq = opts[2] * opts[2], eps3_sq = opts[3] * opts[3], eps4_sq =
          opts[4] * opts[4], eps5 = opts[5];
  double init_p_eL2;
  ...
  if (itmax == 0)
    { /* verify jacobian */
      sba_motstr_chkjac_x(func, fjac, p, &idxij, rcidxs, rcsubs, mcon, cnp,
          pnp, mnp, adata, jac_adata);
      retval = 0;
          ...
          for (j = 0; j < nnz; ++j)
            {
              ...
              ptr2 = wght + k;
              if (!sba_mat_cholinv(ptr1, ptr2, mnp))
                { /* compute w_x_ij s.t. w_x_ij^T w_x_ij = cov_x_ij^-1 */
                  fprintf(
                      stderr,
                      "SBA: invalid covariance matrix for x_ij (i=%d, j=%d) in sba_motstr_levmar_x()\n",
                      i, rcsubs[j]);
                  retval = SBA_ERROR;
                  goto freemem_and_return;
                }
          ...
          for (jj = 0; jj < cnp; jj++)
            {
              if (constraints[j].constrained[jj])
                {
                  double diff = constraints[j].constraints[jj]
                      - p[j * cnp + jj];
          ...
          if (point_constraints[i].constrained)
            {
              for (ii = 0; ii < pnp; ii++)
                {
                  double diff = point_constraints[i].constraints[ii] - p[m
                      * cnp + i * pnp + ii];
              ...
              for (jj = 0; jj < cnp; jj++)
                {
                  if (constraints[j].constrained[jj])
                    {
                      double diff = constraints[j].constraints[jj] - p[j * cnp
                          + jj];
                      /* Add to the U matrix */
                      ptr1[jj * cnp + jj]
                          += /*nnz **/constraints[j].weights[jj];
              ...
              for (ii = 0; ii < pnp; ++ii)
                {
                  for (jj = 0; jj < ii; ++jj)
                    {
                      Vout[i * Vsz + ii * pnp + jj] = Vout[i * Vsz + jj * pnp
                          + ii];
                    }
                }
            }
      ...
      if (itno == 0)
        {
          for (i = mcon * cnp, tmp = DBL_MIN; i < nvars; ++i)
            if (diagUV[i] > tmp)
              tmp = diagUV[i]; /* find max diagonal element */
          mu = tau * tmp;
        }
      ...
      while (1)
        {
          ...
          for (i = 0; i < n; ++i)
            {
              ...
              j = sba_symat_invert_Chol(ptr1, pnp);
              matinv = sba_symat_invert_Chol;
              //j=sba_symat_invert_BK(ptr1, pnp); matinv=sba_symat_invert_BK;
              if (!j)
                {
                  fprintf(
                      stderr,
                      "SBA: singular matrix V*_i (i=%d) in sba_motstr_levmar_x(), increasing damping\n",
                      i);
                  goto moredamping;
          ...
          issolved = sba_Axb_Chol(S, E + mcon * cnp, dpa + mcon * cnp, Sdim,
              MAT_STORAGE);
          ...
          if (issolved)
            {
              ...
              if (MY_DEBUG)
                {
                  if (verbose > 1)
                    {
                      printf(
                          "mean reprojection error %g\n",
                          sba_mean_repr_error(n, mnp, x, hx, &idxij, rcidxs,
                              rcsubs));
                      fflush(stdout);
                    }
                }
                      ...
                      if (point_constraints[i].constrained)
                        {
                          for (ii = 0; ii < pnp; ii++)
                            {
                              double diff =
                                  point_constraints[i].constraints[ii] - p[m
                                      * cnp + i * pnp + ii];

                              /* Add to the U matrix */
                              pdp_eL2 += nvis * point_constraints[i].weight
                                  * diff * diff;
                            }
                        }
                    }
                  ...
                  if (verbose > 1)
                    {
                      printf(
                          ...
                          mu, dL != 0.0 ? dF / dL : dF / DBL_EPSILON,
                          p_eL2 / nvis, pdp_eL2 / nvis, p_eL2 / pdp_eL2);
              ...
              if (dL > 0.0 && dF > 0.0)
                { /* reduction in error, increment is accepted */
                  ...
                  if (pdp_eL2 - 2.0 * sqrt(p_eL2 * pdp_eL2) < (eps4_sq - 1.0)
                      * p_eL2)
                    stop = 4;

                  if (max_pct_change < eps5 && itno >= 4)
                    { /* Run at least 4 iters */
                      stop = 8;
                      break;
                    }
      ...
      for (i = 0; i < n; ++i)
        {
          ...
          if (!j)
            {
              fprintf(stderr,
                  "Singular matrix V*_i (i=%d) in sba_motstr_levmar_x()\n", i);
              // exit(1);
              for (ii = 0; ii < pnp; ii++)
                {
                  for (jj = 0; jj < pnp; jj++)
          ...
          for (j = 0; j < nnz; ++j)
            {
              ...
              for (ii = 0; ii < cnp; ++ii)
                {
                  double *ptr1a = Wout + (rcsubs[j] * cnp + ii) * pnp * n + i
                      * pnp;
...
)
{
  ...
  *pdp; /* p + dp, size m*cnp */
  ...
  if (itmax == 0)
    { /* verify jacobian */
      sba_mot_chkjac_x(func, fjac, p, &idxij, rcidxs, rcsubs, mcon, cnp, mnp,
          adata, jac_adata);
      retval = 0;
      goto freemem_and_return;
    }
...
)
{
  ...
  *pdp; /* p + dp, size n*pnp */
  ...
  if (itmax == 0)
    { /* verify jacobian */
      sba_str_chkjac_x(func, fjac, p, &idxij, rcidxs, rcsubs, pnp, mnp, adata,
          jac_adata);
      retval = 0;
      goto freemem_and_return;
    }
          ...
          if (nsolved == n)
            {
              ...
              if (verbose > 1)
                printf(
                    "\ndamping term %8g, gain ratio %8g, errors %8g / %8g = %g\n",
                    mu, dL != 0.0 ? dF / dL : dF / DBL_EPSILON, p_eL2 / nvis,
                    pdp_eL2 / nvis, p_eL2 / pdp_eL2);
                }
