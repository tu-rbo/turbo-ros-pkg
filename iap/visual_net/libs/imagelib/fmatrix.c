...
/* Use RANSAC to estimate an F-matrix */
int estimate_fmatrix_ransac_matches(int num_pts, v3_t *a_pts, v3_t *b_pts, int num_trials, double threshold,
                                    double success_ratio, int essential, double *F)
{
  int i, j, k, idx;
  ...
  for (i = 0; i < num_trials; i++)
  {
    ...
    for (j = 0; j < 8; j++)
    {
      ...
      for (k = 0; k < j; k++)
      {
        if (idx == idxs[k] || (Vx(a_pts[idx]) == Vx(a_pts[idxs[k]]) && Vy(a_pts[idx]) == Vy(a_pts[idxs[k]])
            && Vz(a_pts[idx]) == Vz(a_pts[idxs[k]])) || (Vx(b_pts[idx]) == Vx(b_pts[idxs[k]]) && Vy(b_pts[idx])
            == Vy(b_pts[idxs[k]]) && Vz(b_pts[idx]) == Vz(b_pts[idxs[k]])))
        {
          reselect = 1;
          break;
        }
    ...
    /* Estimate the F-matrix */
    success = estimate_fmatrix_linear(8, r_pts, l_pts, essential, Ftmp, e1_tmp, e2_tmp);
    ...
    /* Check for nan entries */
    if (isnan(Ftmp[0]) || isnan(Ftmp[1]) || isnan(Ftmp[2]) || isnan(Ftmp[3]) || isnan(Ftmp[4]) || isnan(Ftmp[5])
        || isnan(Ftmp[6]) || isnan(Ftmp[7]) || isnan(Ftmp[8]))
    {
...
/* Refine an F-matrix estimate using LM */
void refine_fmatrix_nonlinear_matches(int num_pts, v3_t *r_pts, v3_t *l_pts, double *F0, double *Fout)
{
  double Ftmp[9];
  double U[9], VT[9];
...
int closest_rank2_matrix_ssv(double *Fin, double *Fout, double *U, double *VT)
{
  double S[3], sigma[9], F_rank2[9], tmp[9];
...
int estimate_fmatrix_linear(int num_pts, v3_t *r_pts, v3_t *l_pts, int essential, double *Fout, double *e1, double *e2)
{
  int i;
  v3_t r_c, l_c;
  ...
  H_p[0] = r_scale;
  H_p[3] = 0.0;
  H_p[6] = -r_scale * Vx(r_c);
  H_p[1] = 0.0;
  H_p[4] = r_scale;
  H_p[7] = -r_scale * Vy(r_c);
  H_p[2] = 0.0;
  H_p[5] = 0.0;
  H_p[8] = 1.0;
...
#define FOCAL_LENGTH_STEP 10.0
  for (i = 0; i < 1000; i++)
  {
    ...
    double Etest[9] = {f * f * F[0], f * f * F[1], f * F[2], f * f * F[3], f * f * F[4], f * F[5], f * F[6], f * F[7],
                       F[8]};
...
void estimate_essential_different_focal_lengths(double *F, double *calib1, double *calib2, double *E)
{
  int i, j;
  double best_ratio = DBL_MAX;
...
#define FOCAL_LENGTH_STEP 10.0
  for (i = 0; i < 400; i++)
  {
    ...
    for (j = 0; j < 400; j++)
    {
      ...
      double Etest[9] = {fm * F[0], fm * F[1], f1 * F[2], fm * F[3], fm * F[4], f1 * F[5], f2 * F[6], f2 * F[7], F[8]};
...
void fmatrix_rectify_images(img_t *a, img_t *b, double *F, double *H_a, double *H_b)
{
  ...
  PPT[0] = scale * (w_b * w_b - 1);
  PPT[1] = 0.0;
  PPT[2] = 0.0;
  PPT[3] = 0.0;
  PPT[4] = scale * (h_b * h_b - 1);
  PPT[5] = 0.0;
  PPT[6] = 0.0;
  PPT[7] = 0.0;
  PPT[8] = 0.0;
  ...
  PPT2[0] = scale * (w_a * w_a - 1);
  PPT2[1] = 0.0;
  PPT2[2] = 0.0;
  PPT2[3] = 0.0;
  PPT2[4] = scale * (h_a * h_a - 1);
  PPT2[5] = 0.0;
  PPT2[6] = 0.0;
  PPT2[7] = 0.0;
  PPT2[8] = 0.0;
  ...
  pcpcT[0] = 0.25 * (w_b - 1) * (w_b - 1);
  pcpcT[1] = 0.25 * (w_b - 1) * (h_b - 1);
  pcpcT[2] = 0.5 * (w_b - 1);
  pcpcT[3] = 0.25 * (w_b - 1) * (h_b - 1);
  pcpcT[4] = 0.25 * (h_b - 1) * (h_b - 1);
  pcpcT[5] = 0.5 * (h_b - 1);
  pcpcT[6] = 0.5 * (w_b - 1);
  pcpcT[7] = 0.5 * (h_b - 1);
  pcpcT[8] = 1.0;

  pcpcT2[0] = 0.25 * (w_a - 1) * (w_a - 1);
  pcpcT2[1] = 0.25 * (w_a - 1) * (h_a - 1);
  pcpcT2[2] = 0.5 * (w_a - 1);
  pcpcT2[3] = 0.25 * (w_a - 1) * (h_a - 1);
  pcpcT2[4] = 0.25 * (h_a - 1) * (h_a - 1);
  pcpcT2[5] = 0.5 * (h_a - 1);
  pcpcT2[6] = 0.5 * (w_a - 1);
  pcpcT2[7] = 0.5 * (h_a - 1);
  pcpcT2[8] = 1.0;
  ...
  H_a_r[0] = -F[5] + w2[1] * F[8];
  H_a_r[1] = -w2[0] * F[8] + F[2];
  H_a_r[2] = 0.0;
  H_a_r[3] = -F[2] + w2[0] * F[8];
  H_a_r[4] = -F[5] + w2[1] * F[8];
  H_a_r[5] = 0.0;
  H_a_r[6] = 0.0;
  H_a_r[7] = 0.0;
  H_a_r[8] = 1.0;
