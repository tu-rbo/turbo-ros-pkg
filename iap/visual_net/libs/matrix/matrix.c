...
#if !defined(WIN32) && !defined(__NO_MKL__)
void matrix_product_ipp(int Am, int An, int Bn, const double *A, const double *B, double *R)
{
  ippmMul_mm_64f(A, An * 8, 8, An, Am, B, Bn * 8, 8, Bn, An, R, Bn * 8, 8);
}

void matrix_transpose_product_ipp(int Am, int An, int Bn, const double *A, const double *B, double *R)
{
  ippmMul_tm_64f(A, An * 8, 8, An, Am, B, Bn * 8, 8, Bn, Am, R, Bn * 8, 8);
}

void matrix_transpose_product2_ipp(int Am, int An, int Bn, const double *A, const double *B, double *R)
{
  ippmMul_mt_64f(A, An * 8, 8, An, Am, B, An * 8, 8, An, Bn, R, Bn * 8, 8);
}

void matrix_array_product_ipp(int count, int Am, int An, int Bn, const double *A, const double *B, double *R)
{
  ippmMul_mama_64f(A, Am * An * 8, An * 8, 8, An, Am, B, An * Bn * 8, Bn * 8, 8, Bn, An, R, Am * Bn * 8, Bn * 8, 8,
                   count);
}
#endif
...
void matrix_product_old(int Am, int An, int Bm, int Bn, const double *A, const double *B, double *R)
{
  int i, j, k;
...
void matrix_transpose_product_old(int Am, int An, int Bm, int Bn, double *A, double *B, double *R)
{
  int r = An;
  int c = Bn;
  int m = Am;
...
void matrix_transpose_product2_old(int Am, int An, int Bm, int Bn, double *A, double *B, double *R)
{
  int r = Am;
  int c = Bm;
  int m = An;
...
void lmdif_driver2(void *fcn, int m, int n, double *xvec, double tol)
{
  ...
  //#else
  lmdif_(fcn, &m, &n, xvec, fvec, &tol, &tol, &gtol, &maxfev, &epsfcn, diag, &mode, &factor, &nprint, &info, &nfev,
         fjac, &ldfjac, ipvt, qtf, wa1, wa2, wa3, wa4);
  //#endif
...
void lmdif_driver3(void *fcn, int m, int n, double *xvec, double tol, int maxfev, double *H)
{
  int info;
  double *fvec;
  double gtol = 0, epsfcn = 0;
...
void dgelss_driver(double *A, double *b, double *x, int m, int n, int nrhs)
{
  ...
  else
  {
    ...
    dgelss_(&m, &n, &nrhs, Atmp, &lda, btmp, &ldb, s, &rcond, &rank, work, &lwork, &info);
...
void dgelsy_driver(double *A, double *b, double *x, int m, int n, int nrhs)
{
  ...
  else
  {
    ...
    dgelsy_(&m, &n, &nrhs, Atmp, &lda, btmp, &ldb, jpvt, &rcond, &rank, work, &lwork, &info);
...
void dgelsy_driver_transpose(double *A, double *b, double *x, int m, int n, int nrhs)
{
  ...
  else
  {
    ...
    /* Query to find a good size for the work array */
    dgelsy_(&m, &n, &nrhs, A, &lda, btmp, &ldb, jpvt, &rcond, &rank, work, &lwork, &info);
...
{
  double sub1[9] = {u[1], u[2], u[3], v[1], v[2], v[3], w[1], w[2], w[3]};

  double sub2[9] = {u[0], u[2], u[3], v[0], v[2], v[3], w[0], w[2], w[3]};

  double sub3[9] = {u[0], u[1], u[3], v[0], v[1], v[3], w[0], w[1], w[3]};

  double sub4[9] = {u[0], u[1], u[2], v[0], v[1], v[2], w[0], w[1], w[2]};
...
{
  double ident[9];
  double n[9] = {0.0, -axis[2], axis[1], axis[2], 0.0, -axis[0], -axis[1], axis[0], 0.0};
...
#if !defined(NO_CBLAS)
void cblas_dgemm_driver_x(int m1, int n12, int k2, int as, int bs, int cs, double *a, double *b, double *c)
{
  cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, m1, k2, n12, 1.0, a, as, b, bs, 0.0, c, cs);
}

void cblas_dgemm_driver(int m1, int n12, int k2, double *a, double *b, double *c)
{
  cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, m1, k2, n12, 1.0, a, n12, b, k2, 0.0, c, k2);
}

void cblas_dgemm_driver_transpose(int m1, int n12, int k2, double *a, double *b, double *c)
{
  cblas_dgemm(CblasRowMajor, CblasTrans, CblasNoTrans, m1, k2, n12, 1.0, a, m1, b, k2, 0.0, c, k2);
}

void cblas_dgemm_driver_transpose2(int m1, int n12, int k2, double *a, double *b, double *c)
{
  cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasTrans, m1, k2, n12, 1.0, a, n12, b, n12, 0.0, c, k2);
}
#endif /* !WIN32 */
