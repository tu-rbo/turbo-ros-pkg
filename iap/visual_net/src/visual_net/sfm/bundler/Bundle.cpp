...
#include "defines.h"
#include "../libs/imagelib/horn.h"
#include "../libs/imagelib/image.h"
#include "../libs/matrix/matrix.h"
#include "../libs/imagelib/qsort.h"
#include "../libs/imagelib/resample.h"
#include "../libs/sfm-driver/sfm.h"
...
double ComputeRayAngle(v2_t p, v2_t q, const camera_params_t &cam1, const camera_params_t &cam2)
{
  double K1[9], K2[9];
...
void BundlerApp::CheckPointKeyConsistency(const std::vector<ImageKeyVector> pt_views, int *added_order)
{
...
void BundlerApp::ReRunSFM(double *S, double *U, double *V, double *W)
{
  ...
  for (int i = 0; i < num_images; i++)
  {
    if (m_image_data[i].m_camera.m_adjusted)
    {
      ...
      memcpy(cameras[num_init_cams].R, m_image_data[i].m_camera.m_R, sizeof(double) * 9);

      matrix_transpose_product(3, 3, 3, 1, m_image_data[i].m_camera.m_R, m_image_data[i].m_camera.m_t,
      ...
      if (m_constrain_focal)
      {
        ...
        if (m_image_data[i].m_has_init_focal)
        {
          ...
          if (fabs(diff) / m_image_data[i].m_init_focal < 0.4)
          {
            ...
            SetFocalConstraint(m_image_data[i], cameras + num_init_cams);
          }
        }
      }
  ...
  RunSFM(num_pts, num_init_cams, 0, false, cameras, init_pts, added_order, colors, pt_views, 0.0 /*eps2*/, S, U, V, W);
  ...
  for (int i = 0; i < num_init_cams; i++)
  {
    ...
    matrix_product(3, 3, 3, 1, cameras[i].R, cameras[i].t, m_image_data[img].m_camera.m_t);

    matrix_scale(3, 1, m_image_data[img].m_camera.m_t, -1.0, m_image_data[img].m_camera.m_t);
...
double BundlerApp::RunSFM(int num_pts, int num_cameras, int start_camera, bool fix_points,
                          camera_params_t *init_camera_params, v3_t *init_pts, int *added_order, v3_t *colors,
                          std::vector<ImageKeyVector> &pt_views, double eps2, double *S, double *U, double *V,
...
#define MIN_POINTS 20
  ...
  do
  {
    ...
    run_sfm(nz_count, num_cameras, start_camera, vmask, projections, fixed_focal ? 0 : 1, 0,
            m_estimate_distortion ? 1 : 0, 1, init_camera_params, nz_pts,
            (m_use_constraints || m_constrain_focal) ? 1 : 0, (m_use_point_constraints) ? 1 : 0, m_point_constraints,
            m_point_constraint_weight, fix_points ? 1 : 0, m_optimize_for_fisheye, eps2, V, S, U, W);
    ...
    for (int i = 0; i < num_cameras; i++)
    {
      ...
      double K[9] = {init_camera_params[i].f, 0.0, 0.0, 0.0, init_camera_params[i].f, 0.0, 0.0, 0.0, 1.0};
      ...
      if (data.m_known_intrinsics)
      {
        ...
        double k_dist[8] = {0.0, 1.0, 0.0, k[0], 0.0, k[1], 0.0, k[4]};
        double w_2 = 0.5 * data.GetWidth();
        double h_2 = 0.5 * data.GetHeight();
        double max_radius = sqrt(w_2 * w_2 + h_2 * h_2) / init_camera_params[i].K_known[0];

        InvertDistortion(8, 6, 0.0, max_radius, k_dist, init_camera_params[i].k_inv);
      }
      ...
      // for (int j = 0; j < num_keys; j++) {
      for (iter = m_image_data[added_order[i]].m_keys.begin(); iter != m_image_data[added_order[i]].m_keys.end(); iter++)
      {
        ...
        if (key.m_extra >= 0)
        {
          ...
          sfm_project_rd(&(init_camera_params[i]), K, init_camera_params[i].k, init_camera_params[i].R, dt, b, pr,
      ...
      double med = kth_element_copy(num_pts_proj, iround(0.8 /* 0.9 */* num_pts_proj), dists);
...
#define NUM_STDDEV 2.0 // 3.0 // 6.0
      ...
      for (int j = 0; j < num_keys; j++)
      {
        ...
        if (m_use_point_constraints && Vx(m_point_constraints[pt_idx]) != 0.0)
        {
          pt_count++;
          continue;
        }
...
void BundlerApp::SetFocalConstraint(const ImageData &data, camera_params_t *params)
{
 ...
 * model if one exists) */
void BundlerApp::InitializeBundleAdjust(int &num_init_cams, int *added_order, int *added_order_inv,
                                        camera_params_t *cameras, v3_t *points, v3_t *colors,
                                        std::vector<ImageKeyVector> &pt_views, bool use_constraints)
{
  int num_images = GetNumImages();
  ...
  {
    std::vector<Keypoint>::iterator iter;
    for (iter = m_image_data[i].m_keys.begin(); iter != m_image_data[i].m_keys.end(); iter++)
    {
  ...
  for (int i = 0; i < num_images; i++)
  {
    if (m_image_data[i].m_camera.m_adjusted)
    {
      ...
      if (m_estimate_distortion)
      {
        ...
        k[0] = cameras[num_init_cams].k[0] = m_image_data[i].m_camera.m_k[0];
        k[1] = cameras[num_init_cams].k[1] = m_image_data[i].m_camera.m_k[1];

        double k_dist[6] = {0.0, 1.0, 0.0, k[0], 0.0, k[1]};
        double w_2 = 0.5 * m_image_data[i].GetWidth();
        double h_2 = 0.5 * m_image_data[i].GetHeight();
        double max_radius = sqrt(w_2 * w_2 + h_2 * h_2) / cameras[num_init_cams].f;

        InvertDistortion(6, 6, 0.0, max_radius, k_dist, cameras[num_init_cams].k_inv);
      }

      if (m_image_data[i].m_known_intrinsics)
      {
        memcpy(cameras[num_init_cams].K_known, m_image_data[i].m_K, 9 * sizeof(double));
        memcpy(cameras[num_init_cams].k_known, m_image_data[i].m_k, 5 * sizeof(double));

        double *k = cameras[num_init_cams].k_known;
        double k_dist[8] = {0.0, 1.0, 0.0, k[0], 0.0, k[1], 0.0, k[4]};
        double w_2 = 0.5 * m_image_data[i].GetWidth();
        double h_2 = 0.5 * m_image_data[i].GetHeight();
        double max_radius = sqrt(w_2 * w_2 + h_2 * h_2) / cameras[num_init_cams].K_known[0];
        // cameras[num_init_cams].f;

        InvertDistortion(8, 6, 0.0, max_radius, k_dist, cameras[num_init_cams].k_inv);
      }
  ...
  {
    points[i] = v3_new(m_point_data[i].m_pos[0], m_point_data[i].m_pos[1], m_point_data[i].m_pos[2]);
    // -m_point_data[i].m_pos[2]);

    colors[i] = v3_new(m_point_data[i].m_color[0], m_point_data[i].m_color[1], m_point_data[i].m_color[2]);
    ...
    for (int j = 0; j < num_views; j++)
    {
      ...
      if (p.second < 0 || p.second >= (int)m_image_data[p.first].m_keys.size())
      {
...
void BundlerApp::SetupProjections(int num_cameras, int num_points, int *added_order, v2_t *projections, char *vmask)
{
  ...
  for (int i = 0; i < num_cameras; i++)
  {
    ...
    for (int j = 0; j < num_keys; j++)
    {
      ...
      if (pidx >= 0)
      {
        ...
        projections[pidx * num_cameras + i] = v2_new(GetKey(idx, j).m_x, GetKey(idx, j).m_y);
      }
    }
  }
}

int BundlerApp::FindCameraWithMostConnectivity(int num_cameras, int num_points, int *added_order, int &parent_idx,
                                               int &max_matches)
{
  int num_images = GetNumImages();
  ...
  for (int i = 0; i < num_images; i++)
  {
    ...
    for (int j = 0; j < num_cameras; j++)
    {
      ...
      const std::vector<KeypointMatch> &list = m_matches.GetMatchList(base);
...
int BundlerApp::FindCameraWithMostMatches(int num_cameras, int num_points, int *added_order, int &parent_idx,
                                          int &max_matches, const std::vector<ImageKeyVector> &pt_views)
{
  max_matches = 0;
...
std::vector<ImagePair> BundlerApp::FindCamerasWithNMatches(int n, int num_cameras, int num_points, int *added_order,
 ...
 * adjustment */
void BundlerApp::BundlePickInitialPair(int &i_best, int &j_best, bool use_init_focal_only)
{
  ...
  for (int i = 0; i < num_images; i++)
  {
    ...
    if (use_init_focal_only && m_use_focal_estimate && !m_image_data[i].m_has_init_focal)
      continue;
    ...
    for (int j = i + 1; j < num_images; j++)
    {
      ...
      if (use_init_focal_only && m_use_focal_estimate && !m_image_data[j].m_has_init_focal)
        continue;
...
int BundlerApp::SetupInitialCameraPair(int i_best, int j_best, double &init_focal_length_0,
                                       double &init_focal_length_1, camera_params_t *cameras, v3_t *points,
                                       v3_t *colors, std::vector<ImageKeyVector> &pt_views)
{
  ...
  //Max: removed because segfault and we have no image
  ...
  if (m_factor_essential && m_image_data[i_best].m_has_init_focal && m_image_data[j_best].m_has_init_focal
      && !m_use_constraints)
  {
...
#define INITIAL_DEPTH 3.0 // 1000.0 // 3.0 /* was 3.0, fixme! */
  ...
  for (unsigned int i = 0; i < num_matches; i++)
  {
    ...
    else
    {
      ...
      bool in_front = true;
      double angle = 0.0;
      points[pt_count] = Triangulate(p, q, cameras[0], cameras[1], error, in_front, angle, true);
...
void BundlerApp::EstimateIgnoredCameras(int &curr_num_cameras, camera_params_t *cameras, int *added_order,
                                        int &curr_num_pts, v3_t *points, v3_t *colors,
                                        std::vector<ImageKeyVector> &pt_views)
{
  int num_images = GetNumImages();
  ...
  for (int i = 0; i < num_images; i++)
  {
    if (m_image_data[i].m_ignore_in_bundle)
    {
      ...
      bool init_success;
      camera_params_t camera = BundleInitializeImage(m_image_data[i], i, curr_num_cameras, curr_num_cameras,
                                                     curr_num_pts, added_order, points, NULL, cameras, pt_views,
                                                     &init_success, false);
...
#if 1
  /* TEST */
  RunSFM(curr_num_pts, curr_num_cameras, 0, true, cameras, points, added_order, colors, pt_views, 1.0e-20 /*eps*/);
#endif

  int pt_count = BundleAdjustAddAllNewPoints(curr_num_pts, curr_num_cameras, added_order, cameras, points, colors, 0.0,
                                             pt_views, 16.0, 2);
  ...
  for (int i = 0; i < num_images; i++)
  {
    if (m_image_data[i].m_ignore_in_bundle && !m_image_data[i].m_camera.m_adjusted)
    {
      printf("[BundleAdjust] Adding camera %d\n", i);
      added_order[curr_num_cameras] = i;
...
#if 1
  /* TEST */
  RunSFM(curr_num_pts, curr_num_cameras, 0, true, cameras, points, added_order, colors, pt_views, 1.0e-20);
#endif

  pt_count = BundleAdjustAddAllNewPoints(curr_num_pts, curr_num_cameras, added_order, cameras, points, colors, 0.0,
                                         pt_views, 16.0, 2);
  ...
  /* Run a final bundle */
  RunSFM(pt_count, curr_num_cameras, 0, false, cameras, points, added_order, colors, pt_views, 1.0e-20);
  ...
  for (int i = 0; i < curr_num_cameras; i++)
  {
    if (m_image_data[added_order[i]].m_has_init_focal)
    {
      printf("   [%03d] %0.3f (%0.3f) %s %d; %0.3e, %0.3e\n", i, cameras[i].f,
             m_image_data[added_order[i]].m_init_focal, m_image_data[added_order[i]].m_name, added_order[i],
             cameras[i].k[0], cameras[i].k[1]);
    }
    else
    {
...
void BundlerApp::BundleAdjust()
{
  ...
  if (num_init_cams == 0)
  {
    ...
    double error0;
    error0 = RunSFM(curr_num_pts, 2, 0, false, cameras, points, added_order, colors, pt_views, 0.0, S, NULL, NULL,
                    NULL, !m_fix_necker);
  ...
  for (int round = curr_num_cameras; round < num_images; round++, curr_num_cameras++)
  {
    ...
    if (m_construct_max_connectivity)
      next_idx = FindCameraWithMostConnectivity(round, curr_num_pts, added_order, parent_idx, max_matches);
    else
      next_idx = FindCameraWithMostMatches(round, curr_num_pts, added_order, parent_idx, max_matches, pt_views);
    ...
    printf("[BundleAdjust[%d]] Adjusting camera %d "
      "(parent = %d, matches = %d)\n", round, next_idx, (parent_idx == -1 ? -1 : added_order[parent_idx]), max_matches);
    ...
    BundleInitializeImage(m_image_data[next_idx],
        ...
        cameras, pt_views);
#else
    BundleInitializeImageFullBundle(next_idx, parent_idx, round, curr_num_pts, added_order, cameras, points, colors,
                                    pt_views);
#endif
    ...
    {
      pt_count = BundleAdjustAddAllNewPoints(curr_num_pts, curr_num_cameras + 1, added_order, cameras, points, colors,
                                             dist0, pt_views);
    }
    ...
    RunSFM(curr_num_pts, round + 1, 0, false, cameras, points, added_order, colors, pt_views);
...
std::vector<int> RefineCameraParameters(const ImageData &data, int num_points, v3_t *points, v2_t *projs, int *pt_idxs,
                                        camera_params_t *camera, double *error_out, bool adjust_focal,
                                        bool remove_outliers, bool optimize_for_fisheye, bool estimate_distortion,
                                        double min_proj_error_threshold, double max_proj_error_threshold)
{
  ...
  while (1)
  {
    ...
    camera_refine(num_points_curr, points_curr, projs_curr, camera, adjust_focal ? 1 : 0, estimate_distortion ? 1 : 0);
    ...
    double med = kth_element_copy(num_points_curr, iround(0.95 * num_points_curr), errors);
...
double BundlerApp::RefinePoints(int num_points, v3_t *points, v2_t *projs, int *pt_idxs, camera_params_t *cameras,
                                int *added_order, const std::vector<ImageKeyVector> &pt_views,
                                camera_params_t *camera_out)
{
  ...
  for (int i = 0; i < num_points; i++)
  {
    ...
    points[i] = triangulate_n_refine(points[i], num_views, pv, Rs, ts, &error_curr);

    v2_t pr = sfm_project_final(camera_out, points[i], 1, m_estimate_distortion ? 1 : 0);
...
std::vector<int> BundlerApp::RefineCameraAndPoints(const ImageData &data, int num_points, v3_t *points, v2_t *projs,
                                                   ...
                                                   camera_params_t *camera_out, bool remove_outliers)
{
  ...
  do
  {
    ...
    // do {
    {
      ...
      /* Refine the camera */
      RefineCameraParameters(data, num_points_curr, points_curr, projs_curr, pt_idxs_curr, camera_out, &error,
                             !m_fixed_focal_length,
                             false, // true,
                             m_optimize_for_fisheye, m_estimate_distortion, m_min_proj_error_threshold,
                             m_max_proj_error_threshold);

#if 1 /* Change to 0 to skip point polishing */
      /* Refine the points */
      error = RefinePoints(num_points_curr, points_curr, projs_curr, pt_idxs_curr, cameras, added_order, pt_views,
                           camera_out);
    ...
    if (remove_outliers)
    {
      ...
      std::vector<int> inliers;
      inliers = RefineCameraParameters(data, num_points_curr, points_curr, projs_curr, pt_idxs_curr, camera_out, NULL,
                                       !m_fixed_focal_length, true, m_optimize_for_fisheye, m_estimate_distortion,
                                       m_min_proj_error_threshold, m_max_proj_error_threshold);
    ...
    {
      RefineCameraParameters(data, num_points_curr, points_curr, projs_curr, pt_idxs_curr, camera_out, NULL,
                             !m_fixed_focal_length, false, m_optimize_for_fisheye, m_estimate_distortion,
                             m_min_proj_error_threshold, m_max_proj_error_threshold);
    }
...
bool FindAndVerifyCamera(int num_points, v3_t *points_solve, v2_t *projs_solve, int *idxs_solve, double *K, double *R,
                         double *t, double proj_estimation_threshold, double proj_estimation_threshold_weak,
                         std::vector<int> &inliers, std::vector<int> &inliers_weak, std::vector<int> &outliers)
{
  ...
  if (num_points >= 9)
  {
    r = find_projection_3x4_ransac(num_points, points_solve, projs_solve, P, /* 2048 */4096 /* 100000 */,
  ...
  double Rigid[12] = {Rinit[0], Rinit[1], Rinit[2], tinit[0], Rinit[3], Rinit[4], Rinit[5], tinit[1], Rinit[6],
                      Rinit[7], Rinit[8], tinit[2]};
  ...
  for (int j = 0; j < num_points; j++)
  {
    ...
    double diff = (pimg[0] - Vx(projs_solve[j])) * (pimg[0] - Vx(projs_solve[j])) + (pimg[1] - Vy(projs_solve[j]))
...
camera_params_t BundlerApp::BundleInitializeImage(ImageData &data, int image_idx, int camera_idx, int num_cameras,
                                                  int num_points, int *added_order, v3_t *points,
                                                  camera_params_t *parent, camera_params_t *cameras,
                                                  std::vector<ImageKeyVector> &pt_views, bool *success_out,
                                                  bool refine_cameras_and_points)
{
  ...
  for (int i = 0; i < num_tracks; i++)
  {
    ...
    {
      projs_solve[num_pts_solve] = v2_new(data.m_keys[key].m_x, data.m_keys[key].m_y);
    }
  ...
  bool success = FindAndVerifyCamera(num_pts_solve, points_solve, projs_solve, idxs_solve, Kinit, Rinit, tinit,
                                     m_projection_estimation_threshold, 16.0 * m_projection_estimation_threshold, /*4.0*/
  ...
  {
    inliers = RefineCameraAndPoints(data, num_points_final, points_final, projs_final, idxs_final, cameras,
                                    added_order, pt_views, &camera_new, true);
  }
  else
  {
    inliers = RefineCameraParameters(data, num_points_final, points_final, projs_final, idxs_final, &camera_new, NULL,
                                     !m_fixed_focal_length, true, m_optimize_for_fisheye, m_estimate_distortion,
                                     m_min_proj_error_threshold, m_max_proj_error_threshold);
  }
  ...
  //Max: removed becaous we do not need the colcor
  //    data.ReadKeyColors();
...
void BundlerApp::BundleInitializeImageFullBundle(int image_idx, int parent_idx, int num_cameras, int num_points,
                                                 ...
                                                 v3_t *colors, std::vector<ImageKeyVector> &pt_views)
{
  ...
  for (int i = 0; i < num_cameras; i++)
  {
    ...
    for (unsigned int j = 0; j < list.size(); j++)
    {
      ...
      else
      {
        ...
        {
          projs_solve[num_pts_solve] = v2_new(GetKey(image_idx, this_idx).m_x, GetKey(image_idx, this_idx).m_y);
        }
  ...
  if (success)
  {
    ...
    matrix_transpose_product(3, 3, 3, 1, Rinit, tinit, cameras[num_cameras].t);
    matrix_scale(3, 1, cameras[num_cameras].t, -1.0, cameras[num_cameras].t);
  ...
  else
  {
    ...
    if (parent_idx != -1)
    {
      memcpy(cameras[num_cameras].R, cameras[parent_idx].R, 9 * sizeof(double));
      memcpy(cameras[num_cameras].t, cameras[parent_idx].t, 3 * sizeof(double));
    }
    else
    {
      printf("[BundleInitializeImageFullBundle] "
    ...
    if (m_use_focal_estimate)
    {
      if (m_image_data[image_idx].m_has_init_focal)
      {
        ...
        if (ratio < 1.4 || m_trust_focal_estimate)
        {
          ...
          if (m_constrain_focal)
            SetFocalConstraint(m_image_data[added_order[num_cameras]], cameras + num_cameras);
        }
        else
        {
          printf("[BundleInitializeImageFullBundle] "
  ...
  {
    /* Run sfm again to update parameters */
    RunSFM(num_points, num_cameras + 1, 0, false, cameras, points, added_order, colors, pt_views);
  }
}
...
std::vector<KeypointMatch> RemoveDuplicateMatches(const std::vector<KeypointMatch> &matches)
{
...
#define NUM_NNS 20
    ...
    v3_t q = v3_new(data.m_drop_pt[0], data.m_drop_pt[1], data.m_drop_pt[2]);
#ifdef __USE_ANN__
    float dists[NUM_NNS];
    ...
    {
      int cam_idx = GetRegisteredCameraIndex(nn_idxs[i]);
      int num_visible_points = m_image_data[cam_idx].m_visible_points.size();
    ...
    for (int i = 0; i < NUM_NNS; i++)
    {
      ...
      matches = MatchKeys(data.m_keys_desc, m_image_data[cam_idx].m_keys_desc, true, 0.75);

      KeypointMatchList matches_sym;
      matches_sym = MatchKeys(m_image_data[cam_idx].m_keys_desc, data.m_keys_desc, false, 1.0);
      ...
      double F[9];
      std::vector<int> inliers = EstimateFMatrix(data.m_keys_desc, m_image_data[cam_idx].m_keys_desc, matches,
    ...
    for (int i = 0; i < NUM_NNS; i++)
    {
      ...
      for (int j = 0; j < (int)match_lists[i].size(); j++)
      {
        ...
        else
        {
          ...
          points_solve[num_pts_solve] = v3_new(pt[0], pt[1], pt[2]);
          ...
          {
            projs_solve[num_pts_solve] = v2_new(data.m_keys_desc[idx1].m_x, data.m_keys_desc[idx1].m_y);
          }
  ...
  { /* !init_location */
...
#else
    std::vector<KeypointMatch> matches = MatchPointsToKeys(data.m_keys_desc, 0.7);
#endif
    ...
    for (int i = 0; i < num_pts_solve; i++)
    {
      ...
      {
        projs_solve[i] = v2_new(data.m_keys_desc[key_idx].m_x, data.m_keys_desc[key_idx].m_y);
      }
  ...
  std::vector<int> inliers, inliers_weak, outliers;
  bool success = FindAndVerifyCamera(num_pts_solve, points_solve, projs_solve, idxs_solve, Kinit, Rinit, tinit,
                                     m_projection_estimation_threshold, 2.0 * m_projection_estimation_threshold,
                                     inliers, inliers_weak, outliers);
...
#if 1
  inliers_final = RefineCameraParameters(data, num_points_final, points_final, projs_final, NULL, camera_new, NULL,
                                         !m_fixed_focal_length, true, m_optimize_for_fisheye, m_estimate_distortion,
                                         m_min_proj_error_threshold, m_max_proj_error_threshold);
#else
  inliers =
...
int BundlerApp::RemoveBadPointsAndCameras(int num_points, int num_cameras, int *added_order, camera_params_t *cameras,
  ...
  for (int i = 0; i < num_points; i++)
  {
    ...
    if (RAD2DEG(max_angle) < 0.5 * m_ray_angle_threshold)
    {
      printf("[RemoveBadPointsAndCamera] "
        "Removing point %d with angle %0.3f and threshold %0.3f\n", i, RAD2DEG(max_angle),
             (0.5 * m_ray_angle_threshold));
