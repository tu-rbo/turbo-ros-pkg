...
#include "../libs/imagelib/defines.h"
...
void TransformInfo::ReadFromFile(FILE *f)
{
  ...
  fscanf(f, "%lf %lf %lf "
    "%lf %lf %lf "
    "%lf %lf %lf", m_H + 0, m_H + 1, m_H + 2, m_H + 3, m_H + 4, m_H + 5, m_H + 6, m_H + 7, m_H + 8);

  /* F-Matrix */
  fscanf(f, "%lf %lf %lf "
    "%lf %lf %lf "
    "%lf %lf %lf", m_fmatrix + 0, m_fmatrix + 1, m_fmatrix + 2, m_fmatrix + 3, m_fmatrix + 4, m_fmatrix + 5,
...
void TransformInfo::WriteToFile(FILE *f)
{
  ...
  fprintf(f, "%0.6le %0.6le %0.6le "
    "%0.6le %0.6le %0.6le "
    "%0.6le %0.6le %0.6le\n", m_H[0], m_H[1], m_H[2], m_H[3], m_H[4], m_H[5], m_H[6], m_H[7], m_H[8]);

  /* F-Matrix */
  fprintf(f, "%0.6le %0.6le %0.6le "
    "%0.6le %0.6le %0.6le "
    "%0.6le %0.6le %0.6le\n", m_fmatrix[0], m_fmatrix[1], m_fmatrix[2], m_fmatrix[3], m_fmatrix[4], m_fmatrix[5],
          m_fmatrix[6], m_fmatrix[7], m_fmatrix[8]);
...
void BundlerApp::ComputeGeometricConstraints(bool overwrite, int new_image_start)
{
...
bool BundlerApp::ComputeTransform(int idx1, int idx2, bool removeBadMatches)
{
  ...
  std::vector<int> inliers = EstimateTransform(m_image_data[idx1].m_keys, m_image_data[idx2].m_keys, list,
                                               MotionHomography, m_homography_rounds, m_homography_threshold,
                                               /* 15.0 *//* 6.0 *//* 4.0 */M);
...
#define MIN_INLIERS 10
  if (num_inliers >= MIN_INLIERS)
  {
    ...
    m_transforms[offset].m_inlier_ratio = ((double)num_inliers) / ((double)list.size());
...
void BundlerApp::ComputeTransforms(bool removeBadMatches, int new_image_start)
{
  ...
  for (unsigned int i = 0; i < num_images; i++)
  {
    ...
    for (iter = m_matches.Begin(i); iter != m_matches.End(i); iter++)
    {
      ...
      {
        matrix_invert(3, m_transforms[idx].m_H, m_transforms[idx_rev].m_H);
      }
    }
  }
...
bool BundlerApp::ComputeEpipolarGeometry(int idx1, int idx2, bool removeBadMatches)
{
  assert(m_image_data[idx1].m_keys_loaded);
  assert(m_image_data[idx2].m_keys_loaded);
  ...
  std::vector<int> inliers = EstimateFMatrix(m_image_data[idx1].m_keys, m_image_data[idx2].m_keys, list,
                                             m_fmatrix_rounds, m_fmatrix_threshold /* 20.0 *//* 9.0 */, F);
...
void BundlerApp::ComputeEpipolarGeometry(bool removeBadMatches, int new_image_start)
{
  ...
  for (unsigned int i = 0; i < num_images; i++)
  {
    ...
    for (iter = m_matches.Begin(i); iter != m_matches.End(i); iter++)
    {
      ...
      bool connect12 = ComputeEpipolarGeometry(i, j, removeBadMatches);
  ...
  for (int i = 0; i < num_points; i++)
  {
    ...
    if (num_views <= 2)
    {
      ...
      for (int j = 0; j < num_views; j++)
      {
        ...
        for (int k = 0; k < 128; k++)
        {
          m_point_data[i].m_desc[k] += GetKeyWithDesc(view, key).m_d[k];
        }
      }
    ...
    else
    {
      ...
      for (int j = 0; j < num_views; j++)
      {
        ...
        for (int k = 0; k < num_views; k++)
        {
          ...
          for (int l = 0; l < 128; l++)
          {
            dist += fabs((double)(GetKeyWithDesc(v1, k1).m_d[l] - GetKeyWithDesc(v2, k2).m_d[l]));
          }
        }
      ...
      for (int k = 0; k < 128; k++)
      {
        m_point_data[i].m_desc[k] = GetKeyWithDesc(best_view, best_key).m_d[k];
      }
    }
  }
}
...
void BundlerApp::CoalesceFeatureDescriptors()
{
  ...
  double median = kth_element(num_points, iround(0.5 * num_points), variance);
...
std::vector<KeypointMatch> BundlerApp::MatchKeysToPoints(const std::vector<KeypointWithDesc> &k1, double ratio)
{
...
std::vector<KeypointMatch> BundlerApp::MatchPointsToKeys(const std::vector<KeypointWithDesc> &keys, double ratio)
{
...
void BundlerApp::RemoveMatchesNearBorder(int i1, int i2, int border_width)
{
  ...
  for (int k = 0; k < num_matches; k++)
  {
    ...
    if (k1.m_x < w1_min || k1.m_x > w1_max || k1.m_y < h1_min || k1.m_y > h1_max || k2.m_x < w2_min || k2.m_x > w2_max
