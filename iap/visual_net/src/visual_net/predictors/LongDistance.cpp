...
LongDistance::LongDistance(double minDistance, double maxDistance) :
  Predictor(string("LongDistance"))
{
  this->minDistance = minDistance;
  this->maxDistance = maxDistance;
}

LongDistance::~LongDistance(void)
{
}

void LongDistance::compute()
{
    ...
    for (int j = i + 1; j < numVertices; j++)
    {
      if (adjMatrix[i][j]->isConnected() && adjMatrix[j][i]->isConnected())
      {
        ...
        if (distA > maxDistance || distB > maxDistance)
        {
          if (distA > maxDistance && distB > maxDistance)
          {
            adjMatrix[i][j]->setCapacity(predictorName, 0 * 0.01); //changed from 0 to 0.5
            adjMatrix[j][i]->setCapacity(predictorName, 0 * 0.01); //changed from 0 to 0.5
          }
          else
          {
            adjMatrix[i][j]->setCapacity(predictorName, 0 * 0.5); //changed from 0 to 0.5
            adjMatrix[j][i]->setCapacity(predictorName, 0 * 0.5); //changed from 0 to 0.5
          }
        }
        else
        {
          // Under the threshold, probability is inverse proportional to distance
          double dist = (distA >= distB ? distA : distB);
          if (dist < minDistance)
          {
          ...
          {
            adjMatrix[i][j]->setCapacity(predictorName,
                                         1.0 - ((double)(dist - minDistance) / (double)(maxDistance - minDistance)));
            adjMatrix[j][i]->setCapacity(predictorName,
                                         1.0 - ((double)(dist - minDistance) / (double)(maxDistance - minDistance)));
          }
        }
      }
